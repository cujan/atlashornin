...
class CiselnikFarbaPresenter extends \BasePresenter
{
	...
	public function  renderEdit($id = 0)
	{
	    ...
	    if (!$form->isSubmitted()){
		$polozka = $this->ciselnikFarbaRepository->findById($id);
		if(!$polozka){
	...
	{
	    $this->template->polozky = $this->ciselnikFarbaRepository->findById($id);
	...
	public function deleteFormSucceeded(){
	    $this->ciselnikFarbaRepository->findById($this->getParameter('id'))->delete();
	...
	protected function createComponentGridCiselnikPodskupina($name) {
	    ...
	    $grid->setModel($this->ciselnikFarbaRepository->findAll());
	...
	public function  nazovFormSubmitted($button)
	{
	    ...
	    if($id){
		$this->ciselnikFarbaRepository->findById($id)->update($values);
		$this->flashMessage('Nazov upraveny');
	    } else {
		$this->ciselnikFarbaRepository->findAll()->insert($values);
