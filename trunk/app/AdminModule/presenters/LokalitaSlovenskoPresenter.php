...
class LokalitaSlovenskoPresenter extends \BasePresenter
{
	...
	public function  renderEdit($id = 0)
	{
	    ...
	    if (!$form->isSubmitted()){
		$polozka = $this->lokalitaSlovenskoRepository->findById($id);
		if(!$polozka){
	...
	{
	    $this->template->polozky = $this->lokalitaSlovenskoRepository->findById($id);
	...
	public function deleteFormSucceeded(){
	    $this->lokalitaSlovenskoRepository->findById($this->getParameter('id'))->delete();
	...
	protected function createComponentGridCiselnikPodskupina($name) {
	    ...
	    $grid->setModel($this->lokalitaSlovenskoRepository->findAll());
	...
	public function  nazovFormSubmitted($button)
	{
	    ...
	    if($id){
		$this->lokalitaSlovenskoRepository->findById($id)->update($values);
		$this->flashMessage('Nazov upraveny');
	    } else {
		$this->lokalitaSlovenskoRepository->findAll()->insert($values);
